...
def get_db_conn():
    ...
    conn = psycopg2.connect(database='Kdd2013AuthorPaperIdentification', user='bhanu')
    return conn
...
def write_submission(predictions, prefix=""):
    ...
    head, tail = os.path.split(submission_path)
    new_path = os.path.join(head, prefix+tail)
    rows = [(author_id, paper_ids_to_string(predictions[author_id])) for author_id in predictions]
    writer = csv.writer(open(new_path, "w"), lineterminator="\n")
